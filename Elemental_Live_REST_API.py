...
class Elemental_API:
    ...
    def getTest(self, ACT, TYPE):
        if TYPE == 'json':        
            try:
                ...
                response = requests.get('http://{}/api/live_events/{}/{}.json'.format(self.IP, self.EVENT_ID, ACT), headers=headers).json()
                status = response
            ...
            try:
                ...
                response = requests.get('http://{}/api/live_events/{}/{}'.format(self.IP, self.EVENT_ID, ACT), headers=headers)
    ...
    def postTest(self, ACT):
        try:
            ...
            response = requests.post('http://{}/api/live_events/{}/{}'.format(self.IP, self.EVENT_ID, ACT), headers=headers, data=datas)
    ...
    def postTest_input_image_inserter(self, ACT, IMAGE_PATH, IMAGE_LAYER, INPUT_ID):
        try:
            ...
            .format(ACT, IMAGE_LAYER, IMAGE_PATH)).encode('utf-8')
            response = requests.post('http://{}/api/live_events/{}/{}/input/{}'.format(self.IP, self.EVENT_ID, ACT, INPUT_ID), headers=headers, data=datas)
            result = (response, response.status_code)
        except Exception as e:
            print('Failed to stop recording file :', e)
        return print(result)
    ...
    def postTest_output_image_inserter(self, ACT, IMAGE_PATH, IMAGE_LAYER, OUTPUT_ID):
        try:
            ...
            "<uri>{2}</uri></image_inserter_input></{0}>"
            .format(ACT, IMAGE_LAYER, IMAGE_PATH)).encode('utf-8')
            response = requests.post('http://{}/api/live_events/{}/{}/output/{}'.format(self.IP, self.EVENT_ID, ACT, OUTPUT_ID), headers=headers, data=datas)
            result = (response, response.status_code)
        except Exception as e:
            print('Failed to stop recording file :', e)
        return print(result)
    ...
    def postTest_replace_playlist_file(self, ACT, File_path):
        try:
            ...
            print(datas)
            response = requests.post('http://{}/api/live_events/{}/{}'.format(self.IP, self.EVENT_ID, ACT), headers=headers, data=datas)
            print('http://{}/api/live_events/{}/{}'.format(self.IP, self.EVENT_ID, ACT))
            result = (response, response.status_code)
        except Exception as e:
            print('Failed to stop recording file :', e)
        return print(result)
...
#TEST    # Input Source SDI replace ----> 기존에 대기하던 인풋이 모두 사라지고 새로운 SDI를 추가 (현재 동작중인 인풋은 제외)
    def postTest_replace_playlist_sdi(self, ACT, SDI_NUM):
        try:
            headers = self.encAuth(ACT)
            datas = ('<inputs><input><loop_source>true</loop_source><device_input><channel>{}</channel><channel_type>HD-SDI</channel_type></device_input></input></inputs>'
...
#TEST    # Input Source Network replace ----> 기존에 대기하던 인풋이 모두 사라지고 새로운 Network Source를 추가 (현재 동작중인 인풋은 제외)
    ...
    def postTest_add_playlist_sdi(self, ACT, SDI_NUM):
        try:
            headers = self.encAuth(ACT)
            datas = ('<input><device_input><channel>{}</channel><channel_type>HD-SDI</channel_type></device_input></input>'
            .format(SDI_NUM)).encode('utf-8')
            print(datas)
            response = requests.post('http://{}/api/live_events/{}/{}'.format(self.IP, self.EVENT_ID, ACT), headers=headers, data=datas)
            print('http://{}/api/live_events/{}/{}'.format(self.IP, self.EVENT_ID, ACT))
